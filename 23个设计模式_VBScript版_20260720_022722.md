...
### 五、本文用到的 7 个 AxonASP 新特性

AxonASP 为 VBScript 引入了大量现代化特性，本文只用了其中 7 个。以"对 23 个设计模式的地道性是否有帮助"为过滤器，这 7 个中 4 个是核心，3 个是锦上添花。
...
没有这 4 个核心特性，23 个模式中 18 个无法地道实现；没有那 3 个锦上添花的特性，23 个模式照样地道——但代码更安全、更简洁、更易维护。未来 AxonASP 若引入更多通用语言改进（如 short-circuit 求值、带参构造函数等），同样可以用 GoF-23 过滤器评估其优先级，与此处的分类互不干扰。
