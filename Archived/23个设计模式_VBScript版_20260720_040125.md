...
|---|---|---|---|
| 强类型（`As Type`） | 全部 18 个"彻底解决"模式的接口引用和参数声明 | 23 个模式用 Variant 都能实现，类型安全不是模式的地道性问题 | 消除整类运行时类型错误，IDE 智能提示的基础 |
...
没有这 4 个核心特性，23 个模式中 18 个无法地道实现；没有那 4 个锦上添花的特性，23 个模式照样地道——但代码更安全、更简洁、更易维护。未来 AxonASP 若引入更多通用语言改进（如 `AndAlso`/`OrElse` 短路求值、带参构造函数等），同样可以用 GoF-23 过滤器评估其优先级，与此处的分类互不干扰。
