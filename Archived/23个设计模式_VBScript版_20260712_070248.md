...
**Axon VBScript 版妥协说明**：
- AxonASP 的 `Static` 变量已支持对象引用，可以在函数内部保持唯一实例，无需模块级全局变量，封装性优于传统版本。但仍无 `Private` 构造函数，外部仍可 `New Singleton` 绕过单例控制。
---
...
Class ComputerBuilder
    ...
    Public Sub IBuilder_BuildCPU(v As String)
        m_Computer.CPU = v
    End Sub
    Public Sub IBuilder_BuildRAM(v As String)
        m_Computer.RAM = v
    End Sub
    Public Sub IBuilder_BuildDisk(v As String)
        m_Computer.Disk = v
    End Sub
...
Function DuplicateObject(obj As ICloneable) As ICloneable
...
- 无继承机制：无法定义抽象原型基类供子类复用拷贝逻辑，每个具体类仍需自行实现 `Clone`。
---
...
Class RealImage
    ...
    Public Sub IImage_Init(filename As String)
        m_Filename = filename
        Response.Write "【加载大图】" & filename
    End Sub
...
- 无继承机制：代理无法继承自一个公共基类来自动转发方法，每个接口方法仍需在 `ProxyImage` 中手动编写转发逻辑；接口方法越多，代理类的样板代码越冗长。
---
## 第7章 外观模式（Facade）
...
Class PrinterAdapter
    ...
    Public Sub Init(oldPrinter As OldPrinter)
        Set m_OldPrinter = oldPrinter
    End Sub

    Public Sub IPrinter_Print(doc As Document)
        m_OldPrinter.OldPrint doc.Content
    End Sub
End Class
...
Class VectorRenderer
    ...
    Public Sub IRenderer_RenderCircle(radius As Integer)
        Response.Write "矢量引擎绘制半径" & radius & "的圆"
    End Sub
End Class
...
Class Circle
    ...
    Public Sub Init(radius As Integer, renderer As IRenderer)
        m_Radius = radius
        Set m_Renderer = renderer
    End Sub
...
- 无继承机制：经典桥接要求 Abstraction（形状）是抽象基类，由子类（圆、方形）继承并扩展。VBScript 无继承，`Circle` 只是一个普通类，无法形成 Shape 抽象层级；若要支持多种形状，需为每种形状各写一个类并各自持有 `IRenderer`，无法复用基类逻辑。
---
## 第10章 组合模式（Composite）
...
Class Leaf
    ...
    Public Sub IComponent_Operation(indent As String)
        Response.Write indent & "叶子：" & m_Name
    End Sub
    Public Sub IComponent_Add(child As IComponent)
        ' 叶子无子节点，空实现
    End Sub
End Class
...
- 无继承机制：`Leaf` 与 `Composite` 仅通过 `IComponent` 接口共享契约，无法继承公共 `Component` 基类来复用默认实现。
- 无 `For Each` 支持：遍历子节点仍需用下标 `For` 循环配合手动扩容数组，无法像现代语言那样用 `For Each child In m_Children` 简洁迭代。
...
Class MilkDecorator
    ...
    ' 注入被装饰的对象
    Public Sub Init(coffee As ICoffee)
        Set m_Coffee = coffee
    End Sub
...
**Axon VBScript 版妥协说明**：
- 接口机制使装饰器与组件实现同一接口 `ICoffee`，类型兼容、可透明嵌套。装饰器直接持有被包裹对象（`ICoffee` 引用），并在 `ICoffee_Cost`/`ICoffee_Description` 中调用其 `Cost`/`Description` 接口方法，保留了惰性求值特性。残留限制：AxonASP 不支持带参构造函数，无法在 `New` 时注入被包裹对象，仍需通过 `Init` 方法显式注入；且 `Init` 是具体类方法而非接口方法，调用方需在具体类型引用上调用 `Init` 后再赋值给接口变量。
---
## 第12章 享元模式（Flyweight）
...
Class QuickSort
    ...
    ' 快速排序递归实现（教学简化版，不适合大规模数组；VBScript 无尾调用优化，大规模数据可能栈溢出）
    Private Function QuickSortHelper(a, lo As Integer, hi As Integer)
...
**Axon VBScript 版妥协说明**：
- `ISortStrategy` 接口保证了所有策略类都有 `Sort` 方法，调用方可以通过接口类型安全地切换策略。`Sorter` 持有 `ISortStrategy` 引用，直接调用 `m_Strategy.Sort` 即可自动路由到具体策略实现，无需完整限定名。残留限制：AxonASP 无泛型，`Sort` 的参数与返回值只能用无类型（Variant）数组，编译期无法约束数组元素类型。
---
## 第14章 观察者模式（Observer）
...
Class Newspaper
    ...
    ' 收到新闻时的响应
    Public Sub ReceiveNews(news As String)
        Response.Write Name & " 收到新闻：" & news
    End Sub
End Class
...
Class PDFExtractor
    ...
    Public Function IExtractor_Extract(data As String)
        Response.Write "【解析 PDF】"
    End Function
End Class
...
Class DataMiner
    ...
    Public Sub ITemplateMiner_SetExtractor(extractor As IExtractor)
        Set m_Extractor = extractor
    End Sub

    Public Sub ITemplateMiner_MineData(filePath As String)
        Response.Write "加载文件：" & filePath
...
**Axon VBScript 版妥协说明**：
- 接口 + 组合实现了流程骨架与可变步骤的分离。`DataMiner` 持有 `IExtractor` 引用，在 `ITemplateMiner_MineData` 中直接调用 `m_Extractor.Extract` 即可自动路由到具体提取器实现，无需辅助方法。`ITemplateMiner` 接口增加了 `SetExtractor` 方法用于注入提取器。残留限制：AxonASP 无继承，无法用"抽象基类 + 钩子方法"的经典模板方法实现，只能用接口 + 组合模拟；且 `SetExtractor` 必须在 `MineData` 之前调用，否则 `m_Extractor` 为 `Nothing` 运行时报错，编译期无法保证调用顺序。
---
## 第16章 迭代器模式（Iterator）
...
Class MyCollection
    ...
    Public Function ICollection_Item(index As Integer)
...
Class MyIterator
    ...
    Public Sub Init(collection As ICollection)
        Set m_Collection = collection
        m_Index = 0
    End Sub
...
**Axon VBScript 版妥协说明**：
- 接口机制（`IIterator`/`ICollection`）解决了多态与契约问题。迭代器持有 `ICollection` 引用，在 `HasNext`/`NextItem` 中直接调用集合的 `Count`/`Item` 接口方法访问元素，无需预取数据，保持了惰性求值。残留限制：AxonASP 无 `IEnumerable` 标准接口，`For Each` 不支持自定义集合，仍需用 `Do While iter.HasNext` 手动循环。
---
...
Class Logger
    ...
    Public Sub IHandler_SetNext(handler As IHandler)
        Set m_Next = handler
    End Sub

    Public Sub IHandler_Handle(msg As String, level As String)
        If ShouldHandle(level) Then
    ...
    Private Function ShouldHandle(level As String)
        Dim levels(2)
        levels(0) = "DEBUG"
        levels(1) = "INFO"
        levels(2) = "ERROR"
        Dim currentIdx As Integer, msgIdx As Integer, i
        currentIdx = -1
        msgIdx = -1
        For i = 0 To 2
...
**Axon VBScript 版妥协说明**：
- `IHandler` 接口约束了链节点的契约。`Logger` 持有 `IHandler` 引用（`m_Next`），在 `IHandler_Handle` 中直接调用 `m_Next.Handle` 转发请求，无需辅助类。残留限制：AxonASP 无继承，无法定义抽象的"基础处理器"基类来复用 `SetNext`/转发逻辑，每个具体处理器都需自行维护 `m_Next` 字段与转发判断；链条的构建（`SetNext` 调用顺序）需调用方保证，编译期无法校验链是否闭合。
---
...
Class LightOnCommand
    ...
    Public Sub Init(light As Light)
        Set m_Light = light
    End Sub
...
Class ChatRoom
    ...
    Public Sub Register(user As User)
        If m_Count >= UBound(m_Users) + 1 Then
            ReDim Preserve m_Users(m_Count * 2)
        End If
        Set m_Users(m_Count) = user
    ...
    Public Sub IMediator_SendMessage(msg As String, fromUser As User)
        Dim i, u As IColleague
        For i = 0 To m_Count - 1
            Set u = m_Users(i)
...
Class User
    ...
    ' 接收消息：显示收到内容
    Public Sub IColleague_Receive(msg As String, fromUser As User)
...
**Axon VBScript 版妥协说明**：
- `IMediator`/`IColleague` 接口约束了中介者和同事的契约。AxonASP 接口方法派发已修复且 `Me` 关键字工作正常，`ChatRoom` 在 `IMediator_SendMessage` 中遍历同事对象数组并直接调用 `IColleague` 接口方法 `Receive`，`User.Send` 通过 `Me` 将自身作为发送者传递给中介者，模式得以自然实现，无需存储用户名或引入辅助类。剩余限制：无继承，中介者职责仍集中在 `ChatRoom`，无法拆分子类；`Register` 为非接口公共方法，未纳入 `IMediator` 契约。
---
...
Class Dot
    ...
    Public Sub IElement_Accept(visitor As IVisitor)
        visitor.VisitDot Me
    End Sub
End Class
...
Class DrawingVisitor
    Implements IVisitor
    Public Sub IVisitor_VisitDot(dot As Dot)
        Response.Write "绘制点：(" & dot.x & "," & dot.y & ")"
    End Sub
    Public Sub IVisitor_VisitCircle(circle As Circle)
        Response.Write "绘制圆：中心(" & circle.x & "," & circle.y & ") 半径" & circle.radius
    End Sub
End Class
...
**Axon VBScript 版妥协说明**：
- `IElement`/`IVisitor` 接口约束了元素和访问者的契约。AxonASP 接口方法派发已修复，元素在 `IElement_Accept` 中可直接调用访问者的 `VisitDot`/`VisitCircle` 接口方法，演示中通过 `IElement` 类型变量调用 `Accept` 即自动路由，无需 `DoAccept` 辅助方法或完整限定名。剩余限制：无方法重载，双分派仍需在 `Accept` 中显式调用 `VisitDot`/`VisitCircle` 分支，新增元素类型时需修改所有访问者。
---
...
Class NumberExpression
    ...
    Public Function IExpression_Interpret(context As Context)
...
Class AddExpression
    ...
    Public Sub Init(left As IExpression, right As IExpression)
        Set m_Left = left
        Set m_Right = right
    End Sub
...
AxonASP 让地道实现的模式从 **3/23** 提升到 **17/23**。接口方法内部调用其他接口方法的限制已修复，上述 14 个"彻底解决"模式均可通过接口引用直接调用接口方法，无需辅助类、辅助函数或完整限定名等变通手段。剩余 6 个模式的残留缺陷（无 Private 构造、无链式调用、无内置 Clone、无 `IEnumerable`、Mediator 职责过重、无方法重载等）属于 VBScript 语言本身的固有局限，与接口方法派发无关。
