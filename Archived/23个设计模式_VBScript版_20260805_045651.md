...
Class MyCollection
    ' 注：内置 Collection 是 COM 对象，As 不支持标注，保留 Variant
    Private m_Items
...
Dim item    ' For Each 循环变量必须为 Variant，语法要求
Set coll = New MyCollection
coll.Add "苹果"
coll.Add "香蕉"
coll.Add "橙子"
...
|---|---|---|---|
| 强类型（`As Type`） | 18 个"彻底解决"模式（迭代器 #16 的 `Count`/`Item(index)`/`coll` 已标注；`m_Items`/`NewEnum`/`For Each` 变量因 COM 限制保留 Variant）+ 3 个"传统已地道"模式的子系统引用、字段与参数 | 23 个模式用 Variant 都能实现，类型安全不是模式的地道性问题 | 消除整类运行时类型错误，IDE 智能提示的基础 |
...
没有这 4 个核心特性，23 个模式中 18 个无法地道实现；没有那 4 个锦上添花的特性，23 个模式照样地道——但代码更安全、更简洁、更易维护。其中强类型现已覆盖全部 21 个地道实现模式（含 3 个"传统已地道"模式），说明"锦上添花"不等于"无足轻重"——它让本就地道的代码更安全、更易维护。未来 AxonASP 若引入更多通用语言改进（如 `AndAlso`/`OrElse` 短路求值、带参构造函数等），同样可以用 GoF-23 过滤器评估其优先级，与此处的分类互不干扰。
