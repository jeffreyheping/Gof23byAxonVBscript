...
Class ChatRoom
    ...
    Public Sub Register(user)
        If m_Count >= UBound(m_Names) + 1 Then
            ReDim Preserve m_Names(m_Count * 2)
        End If
        m_Names(m_Count) = user.Name
        m_Count = m_Count + 1
    End Sub

    ' 非接口公共方法：直接按名称广播，避免存储对象引用
    Public Sub RelayMessage(msg, senderName)
        Dim i
        For i = 0 To m_Count - 1
            If m_Names(i) <> senderName Then
                Response.Write m_Names(i) & " 收到 " & senderName & " 的消息：" & msg & vbCrLf
            End If
        Next
    End Sub

    Public Sub IMediator_SendMessage(msg, fromUser)
        RelayMessage msg, fromUser.Name
    End Sub
End Class
...
Class User
    ...
    Public Sub DoSend(msg)
        m_Mediator.RelayMessage msg, Name
    End Sub
...
**传统 VBScript 版妥协说明**：
- **Property 封装有限**：虽然用 `Property Get/Let` 模拟了封装，但 VBScript 没有 `Friend` 修饰符，外部代码仍然可以读写 `EditorMemento` 的属性。VBA/VB.NET 可以用 `Friend` 让备忘录数据仅对 Originator（TextEditor）可见。
