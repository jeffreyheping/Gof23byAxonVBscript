...
**Axon VBScript 版妥协说明**：
- 接口 + 组合实现了流程骨架与可变步骤的分离。`DataMiner` 持有 `IExtractor` 引用，在 `ITemplateMiner_MineData` 中直接调用 `m_Extractor.Extract` 即可自动路由到具体提取器实现，无需辅助方法。`ITemplateMiner` 接口增加了 `SetExtractor` 方法用于注入提取器。残留限制：无继承，无法用"抽象基类 + 钩子方法"的经典模板方法实现，只能用接口 + 组合模拟；`SetExtractor` 必须在 `MineData` 之前调用，否则 `m_Extractor` 为 `Nothing` 运行时报错，编译期无法保证调用顺序。
---
## 第16章 迭代器模式（Iterator）
...
**Axon VBScript 版妥协说明**：
- `IHandler` 接口约束了链节点的契约。`Logger` 持有 `IHandler` 引用（`m_Next`），在 `IHandler_Log` 中直接调用 `m_Next.Log` 转发请求，无需辅助类。`Enum LogLevel` 替代了传统版的字符串比较，`ShouldHandle` 从 14 行数组查找简化为一行整数比较 `level >= m_Level`，既安全又高效。残留限制：AxonASP 无继承，无法定义抽象的"基础处理器"基类来复用 `SetNext`/转发逻辑，每个具体处理器都需自行维护 `m_Next` 字段与转发判断；链条的构建（`SetNext` 调用顺序）需调用方保证，编译期无法校验链是否闭合。
---
...
> 此模式在 AxonASP 中的实现与传统 VBScript 完全一致。备忘录的核心是状态快照的保存与恢复，传统版已用 `Class EditorMemento` + `Property Get/Let` 妥善实现。虽然 AxonASP 的 UDT（`Type`）看似适合此场景，但 VBA 本身不允许类模块的 Public 方法使用 UDT 作为参数或返回值（`SaveState`/`RestoreState` 均受此限制）——即使 AxonASP 修复其 UDT 实现的 bug 并与 VBA 完全对齐，此限制依然存在。因此直接沿用传统版本即可。
...
| 17 | 责任链 | `Implements` + `Enum` | `IHandler` 链节点契约，`LogLevel` 枚举替代魔法数字 |
...
| 限制项 | 说明 | 影响范围 |
|---|---|---|
| **无继承** | 无法定义抽象基类，接口是唯一的多态手段 | 模板方法、桥接、代理、责任链、中介者 |
| **无方法重载** | 同一类不能有同名不同参数的方法 | 访问者（双分派不优雅） |
| **无 short-circuit 求值** | `And`/`Or` 两边始终求值 | 所有条件判断 |
| **无访问控制修饰符** | 无 `Friend`/`Internal` | 备忘录（状态外部可访问） |
| **无 `For Each` 自定义集合** | 不支持 `IEnumerable` 接口 | 迭代器 |
| **无泛型** | 数组元素无法有编译期类型约束 | 策略（数组参数只能用 Variant） |
...
AxonASP 让地道实现的模式从 **3/23** 提升到 **17/23**。强类型（`As Type`）已全面支持基本类型、接口类型和自定义类类型，包括 `Dim x As ConcreteClass`、`Static x As Type`、`Is Nothing` 等此前受限的语法均已修复。上述 14 个"彻底解决"模式均可通过接口引用直接调用接口方法，配合强类型注解实现编译期类型安全，无需辅助类、辅助函数或完整限定名等变通手段。剩余 6 个模式的残留缺陷（无 Private 构造、无链式调用、无内置 Clone、无 `IEnumerable`、Mediator 职责过重、无方法重载等）属于 VBScript 语言本身的固有局限，与接口方法派发和强类型无关。
