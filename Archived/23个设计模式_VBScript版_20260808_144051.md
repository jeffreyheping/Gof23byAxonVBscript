...
VB.NET 实现 `System.ICloneable` 标准接口，手动深拷贝字段。与 Axon 版场景一致，只保留 Resume 类 + Name/Age/Skills 字段。
...
' 演示：克隆后修改副本，原件不受影响
Dim original As New Resume() With {
    .Name = "张三",
    .Age = 25,
    .Skills = {"VBScript", "HTML"}
}

Dim clone As Resume = DirectCast(original.Clone(), Resume)
clone.Name = "李四"
...
VB.NET 拥有 `IEnumerable(Of T)`/`IEnumerator(Of T)` 泛型接口 + `Yield Return` 关键字——这是 .NET 平台迭代器的地道写法，`Yield Return` 让编译器自动生成状态机枚举器类，无需手写 `MoveNext`/`Current`。
...
Public Class MyCollection(Of T)
    ...
    ' ② 泛型枚举器：Yield Return 自动生成 IEnumerator(Of T) 状态机
    Public Iterator Function GetEnumerator() As IEnumerator(Of T) _
        Implements IEnumerable(Of T).GetEnumerator
        For Each item In m_Items
            Yield Return item   ' 编译器自动生成 MoveNext/Current 状态机
        Next
    End Function
...
**VB.NET 版说明**：
- **`Yield Return` 编译器生成状态机**：`Iterator` 方法中的 `Yield Return item` 让编译器自动生成实现了 `IEnumerator(Of T)` 的嵌套类，包含 `MoveNext()`/`Current`/`Dispose()`。对比传统版：手写 `MyIterator` 需要 20+ 行代码（`m_Index`/`Init`/`HasNext`/`NextItem`），还容易出 bug。
...
| 迭代器类 | 需要手写 `MyIterator` 类（20+ 行） | **已消失**，转发内置 Collection 枚举器 | **已消失**，`Yield Return` 编译器自动生成 |
