...
| 4 | 建造者 | `IBuilder` 约束契约 | 链式调用（方法返回 `Me` 语法糖） | Go 手动 `return b` 实现链式，与 VBScript 相同 |
| 5 | 原型 | `ICloneable` 保证契约 | 内置深拷贝机制 | Go 无内置 Clone，同样手动实现 |
| 16 | 迭代器 | `IIterator` 统一接口 | `For Each` 自定义集合迭代 | Go 用 `range` 遍历，需实现 `Iterable` 接口（issue #52 进行中） |
| 20 | 中介者 | `IMediator`/`IColleague` 约束契约 | （模式本身特点，非语言缺陷） | Go 实现同样集中，靠拆分接口缓解 |
| 21 | 访问者 | `IVisitor`/`IElement` 约束契约 | 方法重载或双分派 | Go 用 type switch 实现双分派 |

### 四、AxonASP 未引入但全局存在的限制

| 限制项 | 缺失语法点 | Go 是否有 | Go 的替代方案 | AxonASP 的替代方案 |
|---|---|---|---|---|
| **无继承** | 继承或 struct embedding | ❌ 无继承，✅ 有 embedding | struct embedding 自动获得嵌入类型的方法 | 接口 + 组合（当前做法），手动委托转发 |
| **无方法重载** | 同名不同参数的方法 | ❌ 无重载 | type switch 或不同方法名 | 显式分支调用（当前做法） |
| **无 short-circuit 求值** | `AndAlso`/`OrElse` | ✅ `&&`/`\|\|` | — | 用嵌套 `If` 或 `Select Case` 模拟 |
| **无访问控制修饰符** | `Private` 构造函数或类间访问控制 | ✅ 首字母大小写控制可见性 | — | 无替代，VBScript 类成员全公开 |
| **无 `For Each` 自定义集合** | `IEnumerable` 接口或 `NewEnum` 约定 | ✅ `range` + `Iterable` | — | `Do While` 手动循环（issue #52 进行中） |
| **无泛型** | 泛型类型参数 | ✅ 1.18 引入 | 1.17 及之前用 `interface{}` | Variant 数组（等同 Go 1.17 之前） |
| **无带参构造函数** | `New Class(args)` | ✅ 构造函数 | — | `Init` 方法显式注入 |
| **无链式调用语法糖** | 方法返回 `Me` 的语法支持 | ✅ 手动 `return b` | — | 手动 `Set x = obj.Method(): x.Method2()` |
...
AxonASP 让地道实现的模式从 **3/23** 提升到 **17/23**。强类型（`As Type`）已全面支持基本类型、接口类型和自定义类类型，包括 `Dim x As ConcreteClass`、`Static x As Type`、`Is Nothing` 等此前受限的语法均已修复。上述 14 个"彻底解决"模式均可通过接口引用直接调用接口方法，配合强类型注解实现编译期类型安全，无需辅助类、辅助函数或完整限定名等变通手段。

剩余 6 个模式的残留缺陷，对照 Go 语言来看可分为三类：

- **Go 也没有、Go 也没更好办法的**：内置深拷贝（原型模式）、链式调用语法糖（建造者模式）。这些在 Go 中同样靠手动实现，不算语言缺陷。
- **Go 也没有但有替代方案的**：继承（Go 用 struct embedding）、方法重载（Go 用 type switch）。AxonASP 当前用接口+组合+手动委托替代，功能等价但样板代码更多。
- **Go 有而 AxonASP 没有的**：`For Each` 自定义迭代（Go 的 `range`）、short-circuit 求值（Go 的 `&&`/`||`）、访问控制（Go 的首字母大小写）、带参构造函数、泛型。这些是 AxonASP 未来可以考虑引入的方向，其中 `For Each` 已在开发中（issue #52）。

值得注意的是，Go 没有"继承"和"方法重载"这两个被广泛认为"OOP 必备"的特性，却依然能地道地实现全部 23 个 GoF 模式。这说明这些特性对设计模式而言并非必需——接口 + 组合才是核心。AxonASP 已具备接口和组合，方向正确。
